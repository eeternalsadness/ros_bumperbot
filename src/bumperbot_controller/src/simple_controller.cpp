...
SimpleController::SimpleController(const ros::NodeHandle &nh, double wheel_radius, double wheel_separation){
    ...
    vel_sub_ = nh_.subscribe("bumperbot_controller/cmd_vel", 10, &SimpleController::velCallback, this);
...
void SimpleController::velCallback(const geometry_msgs::Twist& msg){
    Eigen::Matrix2d robot_speed;
    robot_speed << msg.linear.x,
                   msg.angular.z;
    
    Eigen::Matrix2d wheel_speed = speed_conversion_.inverse() * robot_speed;
    std_msgs::Float64 right_speed;
    right_speed.data = wheel_speed(0, 0);
    std_msgs::Float64 left_speed;
    left_speed.data = wheel_speed(1, 0);
